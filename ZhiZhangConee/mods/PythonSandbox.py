...
class PythonSandbox:
    ...
    def should_sanitize_submodule(self, submodule_name):
        ...
        return any(dangerous in submodule_name for dangerous in dangerous_modules) and submodule_name.count('.') <= 5

    def sanitize_module(self, module, module_name):
        """清理模块中的危险属性"""
        dangerous_patterns = [
            'os', 'sys', 'subprocess', 'ctypes', 'shutil', 'socket',
            'eval', 'exec', 'file', '__builtins__',
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Call):
                if isinstance(node.func, ast.Name):
                    ...
                    if func_name in ['eval', 'exec', 'open', '__loader__', 'load_module', '__spec__',
                                     '__getattribute__']:
                ...
                if node.module and any(
                        dangerous in node.module for dangerous in ['os', 'sys', 'subprocess', 'ctypes', 'shutil']):
        ...
        try:
            ...
            for node in ast.walk(tree):
                ...
                if isinstance(node, ast.Str):
                    if any(keyword in node.s for keyword in ['__loader__', 'load_module', '__spec__']):
                        return True
        except Exception:
            pass
        return False
    ...
    def execute_code_with_inputs(self, code, inputs):
        """在沙箱中执行代码并模拟输入"""
        if self.ast_detect_loader(code) or self.ast_detect_loader2(code):
            return None, "检测到危险的模块加载模式"
        ...
        def run_code():
            try:
                ...
                sys.stdout = captured_stdout = StringIO()
                sys.stderr = captured_stderr = StringIO()
                sys.stdin = MockInputStringIO(inputs)

                try:
                    global_vars_copy = global_vars.copy()
                    global_vars_copy['input'] = mock_input

                    exec(code, global_vars_copy, local_vars)
                    output = captured_stdout.getvalue()
                    errors = captured_stderr.getvalue()
...
class MockInputStringIO:
    """模拟 stdin 用于 input() 函数"""
